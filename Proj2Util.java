...
public class Proj2Util {
    ...
    public static void printBoard(int key, int width, int height){
        String board = gameUnhasher(key, width, height);
        board=board.replace(' ', '.');
        System.out.printf("Original board string\nS = %s\n", board);
        ...
        for(int i=height-1; i >= 0; i--){
            for(int j=0; j < width; j++){
            	...
            	if(charArray[col+i]==' '){
                	...
                	System.out.printf("%c", charArray[col + i ]);
                }
                col+=height;
            }
            col=0;
